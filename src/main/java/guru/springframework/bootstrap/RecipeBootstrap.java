...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
    	log.info("onApplicationEvent() was called!");
        if (categoryRepository.count() == 0L){
            log.info("Loading Categories");
            loadCategories();
        } else {
        	log.warn("Categories already loaded in DB!");
        }
        ...
        if (unitOfMeasureRepository.count() == 0L){
        	...
        	log.warn("UOM already loaded in DB!");
        }
        
        //recipeRepository.saveAll(getRecipes());
        Optional<Recipe> tableRecipe1Optional = recipeRepository.findByDescription("Perfect Guacamole");
        if(!tableRecipe1Optional.isPresent()){
        	recipeRepository.saveAll(getRecipes());
        	log.debug("Saved all recipes!");
        }  else {
        	log.warn("Recipes already existed!");
        }
        
        /*log.error("########## Reactive Repositories ##########");
        log.error("UOM Count: "+unitOfMeasureReactiveRepository.count().block().toString());
        log.info("Category Reactive Count: "+categoryReactiveRepository.count().block().toString());
        log.info("Recipe Reactive Count: "+recipeReactiveRepository.count().block().toString());*/
        
        log.debug("onApplicationEvent(): Loading Bootstrap Data done!");
    }
